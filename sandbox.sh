...
# Published bcrypt hash of literal "admin" — Argo CD's quickstart docs use
# the same hash; Kargo's bcrypt-based local admin accepts it.
KARGO_ADMIN_PASSWORD_HASH='$2a$10$Zrhhie4vLz5ygtVSaif6o.qN36jgs6vjtMBdM6yrU1FOeiAAMMxOm'
KARGO_TOKEN_SIGNING_KEY="${KARGO_TOKEN_SIGNING_KEY:-iwishtowashmyirishwristwatch}"
...
Argo CD
  ...
  username:  admin
  password:  ${pw:-<argocd-initial-admin-secret not found; retry shortly>}
  CLI:       argocd login ${ARGO_HOST}:${SANDBOX_HTTPS_PORT} --username admin --password '${pw:-<password>}' --grpc-web

Kargo
  URL:       https://${KARGO_HOST}:${SANDBOX_HTTPS_PORT}
  username:  admin
  password:  admin
  CLI:       kargo login https://${KARGO_HOST}:${SANDBOX_HTTPS_PORT} --admin --password 'admin'
...
env overrides:
  ...
  KARGO_TOKEN_SIGNING_KEY     override Kargo JWT signing key
EOF
}
