...
type Op struct {
	...
	Reason string
	// Refresh marks best-effort re-wiring of a file we generated
	// earlier (e.g. image coordinates in values-sandbox.yaml following
	// the build manifest). Refresh ops never raise conflicts: an
	// unmodified previous generation regenerates as usual, but edited
	// or legacy files are kept silently — the user's copy wins without
	// prompts or a conflict exit code.
	Refresh bool
}
...
func planWrite(root string, op Op) PlannedOp {
	...
	case legacy:
		if op.Refresh {
			return PlannedOp{
				Op: op, State: StateOursEdited, Decision: DecisionSkip,
				Why: "refresh skipped — pre-tracking generation, kept as-is",
			}
		}
		return PlannedOp{
	...
	default:
		if op.Refresh {
			return PlannedOp{
				Op: op, State: StateOursEdited, Decision: DecisionSkip,
				Why: "refresh skipped — you edited this file, your copy wins",
			}
		}
		return PlannedOp{
			Op: op, State: StateOursEdited, Decision: DecisionSkip, Conflict: true,
